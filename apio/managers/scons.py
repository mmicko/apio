...
class SCons(object):
    ...
    def build(self, args):
        ...
        return self.run('build', var, board, packages=['scons', 'yosys', 'ice40'])

    @util.command
    def time(self, args):
        var, board = process_arguments(args, self.resources)
        return self.run('time', var, board, packages=['scons', 'ice40'])
    ...
    def upload(self, args, serial_port, ftdi_id, sram):
        ...
        return self.run('upload', var, board, packages=['scons', 'ice40'])
